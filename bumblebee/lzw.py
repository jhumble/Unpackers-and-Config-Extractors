...
class ByteDecoder(object):
    ...
    """
    def __init__(self, minwidth=18):
       """
       """

       self._decoder = Decoder()
       self._unpacker = BitUnpacker(initial_code_size=self._decoder.code_size(), minwidth=minwidth)
       self.remaining = []
...
class BitUnpacker(object):
    ...
    def __init__(self, initial_code_size, minwidth=18):
        """
    ...
    def unpack(self, bytesource):
        ...
        codesize = self._initial_code_size
        while (1 << self.minwidth) < codesize:
            self.minwidth +=  1
        ...
        for nextbit in bytestobits(bytesource):
            ...
            if len(bits) == pointwidth:
                ...
                #print(f'yield 0x{codepoint:08X}')
                yield codepoint
...
class Decoder(object):
    ...
    def _decode_codepoint(self, codepoint):
        ...
        if codepoint in self._codepoints:
            ...
            #print(f'prefix: {self._prefix}, ret: {ret}')
            #from pprint import pprint
            #pprint(self._codepoints)
            if None != self._prefix:
                self._codepoints[ len(self._codepoints) ] = self._prefix + six.int2byte(six.indexbytes(ret, 0))
                #self._codepoints[ len(self._codepoints) ] = self._prefix + self._prefix#six.int2byte(six.indexbytes(ret, 0))
        ...
        #print(f'Codepoint 0x{codepoint:08X} => {ret}')
        return ret
